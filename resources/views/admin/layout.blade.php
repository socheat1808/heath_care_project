    ...
    <aside class="sidebar" id="sidebar">
        ...
        <div class="sidebar-section">
            ...
            <a href="{{ route('admin.doctors.doctor-approvals') }}"
                ...
                Doctor Approvals
                @php $pendingDoctors = \App\Models\User::where('role','doctor')->where('status','pending')->count(); @endphp
                @if($pendingDoctors > 0)
                <span class="nav-badge">{{ $pendingDoctors }}</span>
                @endif
            </a>
            ...
            {{-- Leave Requests ← NOW INSIDE MANAGEMENT SECTION --}}
    ...
    <div class="main">
        ...
        <header class="topbar">
            ...
            <div class="topbar-actions">
                ...
                {{-- Bell --}}
                @php $bellCount = \App\Models\Appointment::where('status','pending')->count(); @endphp
                <a href="{{ route('admin.appointments.index', ['status' => 'pending']) }}"
                    class="icon-btn" title="{{ $bellCount }} pending appointments">
                    <svg fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
                        <path stroke-linecap="round" stroke-linejoin="round"
                ...
                <div style="display:flex;align-items:center;gap:.65rem">
                    <div style="text-align:right">
                        <div style="font-size:.83rem;font-weight:600;color:var(--text-primary);line-height:1.2">
                            {{ Auth::user()->name }}
                        </div>
                        <div style="font-size:.72rem;color:var(--text-muted)">Administrator</div>
                    </div>

                    {{-- Avatar --}}
                    <div style="width:34px;height:34px;border-radius:50%;background:var(--primary);
                        color:#fff;display:flex;align-items:center;justify-content:center;
                        font-weight:700;font-size:.75rem;flex-shrink:0;cursor:pointer">
                        {{ strtoupper(substr(Auth::user()->name, 0, 2)) }}
                    </div>
                    ...
                    <form method="POST" action="{{ route('logout') }}" style="margin:0">
                        ...
                        <button type="submit"
                           ...
                           color:var(--text-muted);cursor:pointer;transition:all .15s"
                            ...
                            onmouseout="this.style.borderColor='var(--card-border)';this.style.color='var(--text-muted)';this.style.background='none'">
